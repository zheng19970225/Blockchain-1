...
class Web3jService extends ServiceBase {
  ...
  async getBlockByNumber(blockNumber, includeTransactions) {
    ...
    if (
      parseInt(blockNumber, 10) === NaN &&
      parseInt(blockNumber, 16) === NaN
    ) {
  ...
  async getTransactionByBlockHashAndIndex(blockHash, transactionIndex) {
    ...
    if (
      parseInt(transactionIndex, 10) === NaN &&
      parseInt(transactionIndex, 16) === NaN
    ) {
  ...
  async getCode(address) {
    ...
    let requestData = {
      jsonrpc: '2.0',
      method: 'getCode',
      params: [this.config.groupID, address],
      id: 1,
    };
  ...
  async getSystemConfigByKey(key) {
    ...
    let requestData = {
      ...
      params: [this.config.groupID, key],
      id: 1,
    };
  ...
  async sendRawTransaction(...args) {
    ...
    if (args.length !== 3) {
      ...
      let signTx = await this.rawTransaction(
        to,
        func,
        params,
        blockNumber + 500,
  ...
  async deploy(contractPath, outputDir) {
    ...
    let contractBin = fs.readFileSync(
      path.join(outputDir, contractName + '.bin'),
      'utf-8',
    );
    let blockNumberResult = await this.getBlockNumber();
    let blockNumber = parseInt(blockNumberResult.result, '16');
    let signTx = web3Sync.getSignDeployTx(
      this.config.groupID,
      this.config.account,
      this.config.privateKey,
      contractBin,

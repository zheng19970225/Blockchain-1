...
class CRUDService extends ServiceBase {
  ...
  _checkTableKeyLength(table) {
    if (table.key.length > constant.TABLE_KEY_MAX_LENGTH) {
      throw new Error(
        `the value of the table key exceeds the maximum limit (${constant.TABLE_KEY_MAX_LENGTH})`,
      );
    }
  }
  ...
  async _send(
    ...
    parameters,
    readOnly = false,
    address = constant.CRUD_PRECOMPILE_ADDRESS,
  ) {
  ...
  async createTable(table) {
    ...
    let output = await this._send(
      constant.TABLE_FACTORY_PRECOMPILE_ABI.createTable,
      parameters,
      false,
      constant.TABLE_FACTORY_PRECOMPILE_ADDRESS,
    );
  ...
  async insert(table, entry) {
    ...
    let output = await this._send(
      constant.CRUD_PRECOMPILE_ABI.insert,
      parameters,
    );
  ...
  async update(table, entry, condition) {
    ...
    let output = await this._send(
      constant.CRUD_PRECOMPILE_ABI.update,
      parameters,
    );
    return parseInt(output);
  }
  ...
  async select(table, condition) {
    ...
    let output = await this._send(
      constant.CRUD_PRECOMPILE_ABI.select,
      parameters,
      true,
    );
  ...
  async remove(table, condition) {
    ...
    let output = await this._send(
      constant.CRUD_PRECOMPILE_ABI.remove,
      parameters,
    );
  ...
  async desc(tableName) {
    ...
    if (userTable.length !== 0) {
      let tableInfo = new Table(
        tableName,
        userTable[0].key_field,
        userTable[0].value_field,
      );
      return tableInfo;
    } else {

...
class ConsensusService extends ServiceBase {
  ...
  async _send(abi, nodeID) {
    ...
    let receipt = await this.web3jService.sendRawTransaction(
      constant.CONSENSUS_PRECOMPILE_ADDRESS,
      functionName,
      parameters,
    );
  ...
  async addSealer(nodeID) {
    ...
    if (!isValid) {
      throw new PrecompiledError(
        OutputCode.getOutputMessage(OutputCode.P2PNetwork),
      );
    }
    ...
    if (sealers.includes(nodeID)) {
      throw new PrecompiledError(
        OutputCode.getOutputMessage(OutputCode.SealerList),
      );
    }
  ...
  async addObserver(nodeID) {
    ...
    if (observers.includes(nodeID)) {
      throw new PrecompiledError(
        OutputCode.getOutputMessage(OutputCode.ObserverList),
      );
    }
  ...
  async removeNode(nodeID) {
    ...
    if (!peers.includes(nodeID)) {
      throw new PrecompiledError(
        OutputCode.getOutputMessage(OutputCode.GroupPeers),
      );
    }

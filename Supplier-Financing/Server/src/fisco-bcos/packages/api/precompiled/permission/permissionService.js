...
class PermissionService extends SeviceBase {
  ...
  async _grant(tableName, address) {
    let functionName = utils.spliceFunctionSignature(
      constant.PERMISSION_PRECOMPILE_ABI.insert,
    );
    let parameters = [tableName, address];

    let receipt = await this.web3jService.sendRawTransaction(
      constant.PERMISSION_PRECOMPILE_ADDRESS,
      functionName,
      parameters,
    );
    let result = handleReceipt(
      receipt,
      constant.PERMISSION_PRECOMPILE_ABI.insert,
    )[0];
  ...
  async _revoke(tableName, address) {
    let functionName = utils.spliceFunctionSignature(
      constant.PERMISSION_PRECOMPILE_ABI.remove,
    );
    let parameters = [tableName, address];
  ...
  async _list(tableName) {
    let functionName = utils.spliceFunctionSignature(
      constant.PERMISSION_PRECOMPILE_ABI.queryByName,
    );
    let parameters = [tableName];
    let receipt = await this.web3jService.call(
      constant.PERMISSION_PRECOMPILE_ADDRESS,
      functionName,
      parameters,
    );

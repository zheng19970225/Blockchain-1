...
module.exports.check = function(args, ...types) {
  ...
  if (args.length !== types.length) {
    throw new TypeError(
      `unmatch number of arguments, expected ${types.length} but got ${args.length}`,
    );
  }
  ...
  for (let index in args) {
    if (typeof types[index] === 'number') {
      ...
      if (typeof args[index] !== _types[types[index]]) {
        throw new TypeError(
          `invalid parameter at position ${index}, expected ${
            _types[types[index]]
          } but got ${typeof args[index]}`,
        );
      }
      ...
      if (!(args[index] instanceof types[index])) {
        ...
        throw new TypeError(
          `invalid parameter at position ${index}, expected instance of ${functionName} but got ${typeof args[
            index
          ]}`,
        );
      }

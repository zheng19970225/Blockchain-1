...
class Configuration {
  ...
  constructor() {
      ...
      for (let childProperty of ['key', 'cert', 'ca']) {
        if (!auth[childProperty] || typeof auth[childProperty] !== 'string') {
          throw new ConfigurationError(
            `invalid ${childProperty} property in \`authentication\``,
          );
        }
      ...
      for (let nodeIndex in nodes) {
        ...
        for (let childProperty of ['ip', 'port']) {
          if (!node[childProperty] || typeof node[childProperty] !== 'string') {
            throw new ConfigurationError(
              `invalid ${childProperty} property in \`nodes\` at position ${nodeIndex}`,
            );
          }
        }
      }
    ...
    if (!config.privateKey || typeof config.privateKey !== 'object') {
      ...
      if (
        !privateKey.type ||
        !['pem', 'ecrandom', 'p12'].includes(privateKey.type)
      ) {
      ...
      if (!privateKey.value || typeof privateKey.value !== 'string') {
        throw new ConfigurationError(
          'invalid `value` property in `privateKey`',
        );
      }
      ...
      switch (privateKey.type) {
        ...
        case 'ecrandom': {
          if (privateKey.value.length !== 64) {
            throw new ConfigurationError(
              'the length of private key should be 128 bits',
            );
          }
        ...
        case 'p12': {
          if (!privateKey.password || typeof privateKey.password !== 'string') {
            throw new ConfigurationError(
              'invalid `password` property in `privateKey`',
            );
          }
      ...
      this.account =
        '0x' + web3Utils.privateKeyToAddress(this.privateKey).toString('hex');
    }
  }
}

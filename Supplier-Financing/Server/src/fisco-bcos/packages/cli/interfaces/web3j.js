      ...
      args: [
        ...
        },
        {
          ...
          options: {
            type: 'boolean',
            describe:
              '(optional) If true it returns the full transaction objects, if false only the hashes of the transactions',
            flag: FLAGS.OPTIONAL,
          },
        },
      ],
      ...
      args: [
        {
          ...
          options: {
            type: 'string',
            describe:
              'The name of a contract which must be in `nodejs-sdk/packages/cli/contracts/` directory',
          },
        },
      ],
    },
      ...
      return web3jService.deploy(contractPath, outputDir).then(result => {
        ...
        if (result.status === '0x0') {
          let addressPath = path.join(
            outputDir,
            `.${path.basename(contractName, '.sol')}.address`,
          );
    ...
    argv => {
      ...
      for (let item of abi) {
        if (item.name === functionName && item.type === 'function') {
          ...
          if (item.constant) {
            return web3jService
              .call(contractAddress, functionName, parameters)
              .then(result => {
            ...
            return web3jService
              .sendRawTransaction(contractAddress, functionName, parameters)
              .then(result => {

  ...
  produceSubCommandInfo(
    ...
    argv => {
      return permissionService.listCNSManager().then(cnsManagers => {
        if (
          cnsManagers.length !== 0 &&
          cnsManagers.findIndex(value => value.address === config.account) < 0
        ) {
          throw new Error(
            OutputCode.getOutputMessage(OutputCode.PermissionDenied),
          );
        }
        ...
        return cnsService
          ...
          .then(queryResult => {
            ...
            return web3jService.deploy(contractPath, outputDir).then(result => {
              ...
              let abi = fs
                .readFileSync(path.join(outputDir, `${contractName}.abi`))
                .toString();
    ...
    argv => {
      ...
      return cnsService
        ...
        .then(addressInfo => {
          ...
          let functionIndex = abi.findIndex(
            value => value.type === 'function' && value.name === functionName,
          );
          if (functionIndex < 0) {
            throw new Error(
              `no function named as \`${functionName}\` in contract \`${contractName}\``,
            );
          }
          ...
          if (abi.inputs.length !== parameters.length) {
            throw new Error(
              `wrong number of parameters for function \`${abi.name}\`, expected ${abi.inputs.length} but got ${parameters.length}`,
            );
          }
          ...
          if (abi.constant) {
            return web3jService
              .call(address, functionName, parameters)
              .then(result => {
            ...
            return web3jService
              .sendRawTransaction(address, functionName, parameters)
              .then(result => {

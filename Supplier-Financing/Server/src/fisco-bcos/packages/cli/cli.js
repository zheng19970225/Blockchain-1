...
if (cwd !== dirname) {
  console.error(
    chalk.red(
      '[ERROR]:cli.js must be run in `nodejs-sdk/packages/cli/` directory',
    ),
  );
  process.exit(-1);
}
...
function parseSub(subCommandInfo, argv, originArgv) {
  ...
  if (needHelp) {
    ...
    if (subCommandInfo.args) {
      argv.arguments = argv.arguments.concat(
        Array(subCommandInfo.args.length).fill('?'),
      );
    }
  }

  let parser = yargs(argv.arguments).command(
    command,
    chalk.cyan(subCommandInfo.describe),
    yargs => {
      if (subCommandInfo.args) {
...
function main() {
  let parser = yargs()
    .completion(
      'completion',
      chalk.green('generate completion script'),
      completion(),
    )
    .command('$0 list', chalk.green('list all commands'))
    .command(
      '$0 <command> [arguments...]',
      chalk.green('run a specified command'),
      yargs => {
